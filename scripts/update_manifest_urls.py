    ...
    if current_manifest.startswith("scripts/tmp/"):
        ...
        if local_manifest.exists():
            ...
            if public_url:
                ...
                print(f"Warning: {slug}: Could not extract @id from local manifest")
                return False
        else:
            print(f"Warning: {slug}: Local manifest file not found: {local_manifest}")
            return False
    
    print(f"Warning: {slug}: Manifest path format not recognized: {current_manifest}")
    return False
