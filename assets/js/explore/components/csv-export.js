...
import { getVal, getRes, rawValue } from '../core/utils.js';
import { MAP } from './facets.js';
import { HIDE_FIELDS, LABEL_RENAMES } from './details-panel.js';
...
const access = {
  field: (label, fieldName) => ({
    label,
    get: r => getExportFieldValue(r, fieldName) || getVal(r, fieldName)
  }),
  ...
  details: (label, fieldName) => ({
    label,
    get: r => getExportFieldValue(r, fieldName)
  }),
...
function getExportFieldValue(rec, fieldName) {
  const values = (rec?.details || [])
    .filter(d => d.fieldName === fieldName)
    .map(detailExportValue)
...
export function buildExportFields(entityType, records = []) {
 ...
 */
export function openCSVDialog(elements, entityType, records = []) {
  const { dialog, fieldsContainer } = elements;
  const fields = buildExportFields(entityType, records);
 ...
 */
export function getSelectedFieldAccessors(fieldsContainer, entityType, records = []) {
  const fields = buildExportFields(entityType, records);
